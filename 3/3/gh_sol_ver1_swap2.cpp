//gh_sol_ver1_swap : 11.0sec
//gh_sol_ver1_swap2: 11.0 ~ 12.0sec
//gh_sol_ver1      : 12.6sec
//general tree case에서 오래 걸림.
#include <stdio.h>
#include <string.h>
#include <map>
#include <set>
...
int bfs(int upper_bound)
{
    ...
    */
    ...
    for(map <int, set <int>>::iterator iter = con.begin(); iter != con.end(); iter++)
    {
        ...
        for(set <int>::iterator t_iter = (iter->second).begin(); 
        t_iter != (iter->second).end(); t_iter++)
        {
            int ref_id = (*t_iter);
            //참조가 제거되었다면 pass
