...
class manager{
    ...
    */
    static class gameState{
        private final HashMap<Integer, Integer> resource = new HashMap<>();
        private final HashMap<Integer, Card> keepCard = new HashMap<>();
        private final Deque<Card> deck;
        gameState(Deque<Card> deck){
            this.deck = deck;
        }
        public Card getCurCard(int peopleNum){
            keepCard.computeIfAbsent(peopleNum, k->deck.pollFirst());
            return keepCard.get(peopleNum);
        }

        public void discardCard(int peopleNum) {
            keepCard.remove(peopleNum);
        }

        public void occupyResource(int resourceNum){
            resource.put(resourceNum, 1);
        }
        ...
        public boolean checkOccupyResource(int resourceNum) {
            return resource.containsKey(resourceNum);
        }
    }
}
class Main {
    public static void main(String []args) throws IOException {
        ...
        for(int i=0;i<T;i++){
            line = bf.readLine().split(" ");
            cList.add(new Card(line[1], Integer.parseInt(line[0]),
                    (line.length < 3 ? -1 : Integer.parseInt(line[2]))));
        }
